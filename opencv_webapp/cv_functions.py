...
def cv_detect_face(path): # path parameter를 통해 파일 경로를 받아들이게 됩니다.
   ...
   if (type(img) is np.ndarray):
       ...
       face_cascade = cv2.CascadeClassifier(baseUrl+'models/haarcascade_frontalface_default.xml')
       eye_cascade = cv2.CascadeClassifier(baseUrl+'models/haarcascade_eye.xml')
